...
def load_prepare_data():
    ...
    df_confirmedM = pd.melt(df_confirmed, id_vars=cols_list,value_vars=dates_list, var_name='Date', value_name='Confirmed')
    cols_list = df_deaths.columns.to_list()[:4]
    dates_list = df_deaths.columns.to_list()[4:]
    # tidy data df_deaths 
    df_deathsM = pd.melt(df_deaths, id_vars=cols_list,value_vars=dates_list, var_name='Date', value_name='Deaths')
    # Recovered 
    ...
    df_recoveredM = pd.melt(df_recovered, id_vars=cols_list,value_vars=dates_list, var_name='Date', value_name='Recovered')
    ...
    df_covid19 = pd.melt(df_covid19, id_vars=cols_ids,value_vars=cases, var_name='Cases', value_name='Count')
...
st.markdown("<h1 style='text-align: left; color: red;'>Aberdeen Python User Group Meeting (Nov-2020)</h1>", unsafe_allow_html=True)
...
msg_intro = msg_intro +  ' and last updated on <b>'+str(pd.to_datetime(max(covid19['Date'])).strftime("%d-%m-%y"))+'</b>'
...
start_date = # create date_input on the side bar
end_date = # create date_input on the side bar
number_s = # create number_input on the side bar (for to n countries)
cases_type = # use selectbox to create choices to show either Confirmed, Deaths, or Recovered cases


# call the function top_countries_by_cases_by_date suing the user's input data 

fig = top_countries_by_cases_by_date(number_s,True,end_date,cases_type,' by '+str(end_date.strftime("%d/%m/%y")))
# plot the diagram 
st.plotly_chart(fig) 
